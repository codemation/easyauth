...
from easyadmin import Admin, admin, buttons, card, forms, html_input, modal, row
...
async def frontend_setup(server):
    ...
    server.admin = Admin(
        ...
        side_bar_sections=[
            ...
            {
                "items": [
                    {"name": "APIs", "href": f"/docs", "icon": "flag", "items": []}
                ]
            },
        ],
    )
    ...
    logout_modal = modal.get_modal(
        ...
        body=buttons.get_button("Go Back", color="success", href=f"{admin_prefix}/")
    ...
    @admin_gui.get(
        "/", response_class=HTMLResponse, send_token=True, include_in_schema=False
    )
    ...
    @admin_gui.get(
        "/users", response_class=HTMLResponse, send_token=True, include_in_schema=False
    )
    ...
    async def admin_user_page(username: str, access_token: str = None):
        ...
        if not user:
            raise HTTPException(
                status_code=404, detail=f"No User with name {username} exists"
            )
    ...
    async def admin_service_page(service_name: str, access_token: str = None):
        ...
        if not service:
            raise HTTPException(
                status_code=404, detail=f"No Service with name {service_name} exists"
            )
    ...
    async def get_user_details(user):
        ...
        modal_row = row.get_row(
            card.get_card(f"{user.username}", update_form, size=12)
            + card.get_card(
                f"Groups",
                "".join(
                    [
    ...
    async def _admin_users(access_token: str, account_type="user"):
            ...
            modals.append(
                modal.get_modal(
                    ...
                    body=forms.get_form(
                        ...
                        [
                            buttons.get_button(
                                "Go Back", color="success", href=f"{admin_prefix}/"
                            )
                        ],
            ...
            users_table[ind]["groups"] = "".join(
                [
                    buttons.get_button(
                        group["group_name"],
                        color="success",
                        href=f'{admin_prefix}/group/{group["group_name"]}',
                    )
                    for group in users_table[ind]["groups"]
                ]
            )
            actions = buttons.get_split_button(
                f"view/edit", icon="eye", modal=f"view_{username_normalized}"
        ...
        email_and_full_name_input = html_input.get_text_input(
            "email"
        ) + html_input.get_text_input("full_name")
        ...
        return server.admin.table_page(
            ...
            below=forms.get_form(
                ...
                [
                    ...
                    html_input.get_checkbox(
                        "groups", [(group, False) for group in groups], size=12
                    ),
                ],
    ...
    async def get_group_details(group_name: str):
        ...
        if not group:
            raise HTTPException(
                status_code=404, detail=f"No Group with name {group_name} exists"
            )
        ...
        roles_in_group = [(role, True) for role in roles] + [
        ...
        modal_row = row.get_row(
            card.get_card(f"{group_name}", update_form, size=12)
            + card.get_card(
                f"Users",
                "".join(
                    [
    ...
    @admin_gui.get(
        "/groups", response_class=HTMLResponse, send_token=True, include_in_schema=False
    )
    async def admin_groups(access_token=None):
        ...
        for ind, group in enumerate(groups):
            ...
            actions = buttons.get_split_button(
                f"view/edit", icon="eye", modal=f"view_{group_name}"
            ) + buttons.get_split_button(
                f"delete",
                color="danger",
        ...
        admin_table = server.admin.table_page(
            "Groups",
            groups_table
            if len(groups_table) > 0
            else [{"group_name": "NO GROUPS", "roles": ""}],
            current_user=access_token["permissions"]["users"][0],
            modals="".join(modals),
            above="",
            below=forms.get_form(
                ...
                [
                    ...
                    html_input.get_checkbox(
                        "roles", [(role.role, False) for role in roles], size=12
                    ),
                ],
    ...
    async def get_role_details(role_name: str):
        ...
        if not role:
            raise HTTPException(
                status_code=404, detail=f"No Role with name {role_name} exists"
            )
        ...
        permissions = [
            action.action for action in role_actions if action.action in all_actions
        ]
        ...
        for user in all_users:
            for group in user.groups:
                if group.group_name in groups:
                    ...
                    break
        actions_in_role = [(action, True) for action in permissions] + [
        ...
        update_form = forms.get_form(
            ...
            [
                ...
                html_input.get_checkbox("actions", actions_in_role, size=12)
                if len(actions_in_role) > 0
                else "no actions",
            ],
        ...
        modal_row = row.get_row(
            card.get_card(f"{role_name}", update_form, size=12)
            + card.get_card(
                f"Users",
                "".join(
                    [
    ...
    @admin_gui.get(
        "/roles", response_class=HTMLResponse, send_token=True, include_in_schema=False
    )
        ...
        for role in roles:
            ...
            role["actions"] = "".join(
                [
                    ...
                    for action in role["actions"]
                    if action["action"] in all_actions
                ]
            )

            actions = buttons.get_split_button(
                f"view/edit", icon="eye", modal=f"view_{role_name}"
            ) + buttons.get_split_button(
                f"delete", modal=f"delete{role_name}Modal", color="danger", icon="trash"
            )
            role[" "] = actions
        admin_table = server.admin.table_page(
            "Roles",
            ...
            below=forms.get_form(
                ...
                [
                    ...
                    html_input.get_checkbox(
                        "actions", [(action, False) for action in all_actions], size=12
                    ),
                ],
    ...
    async def get_action_details(action: str):
        ...
        if not action:
            raise HTTPException(
                status_code=404, detail=f"No permission with name {action} exists"
            )
        ...
        modal_row = row.get_row(
            card.get_card(f"{action.action}", update_form, size=12)
            + card.get_card(
                f"Users",
                "".join(
                    [
    ...
    async def admin_action_page(action: str, access_token=None):
        ...
        if not action:
            raise HTTPException(
                status_code=404, detail=f"No action with name {action.action} exists"
            )
    ...
    async def admin_actions(access_token=None):
        ...
        for action in actions_table:
            ...
            view_actions = buttons.get_split_button(
                f"view / edit", icon="eye", modal=f"view_{action['action']}"
            ) + buttons.get_split_button(
                f"delete",
        ...
        return server.admin.table_page(
            "Permissions",
            actions_table
            if len(actions_table) > 0
            else [{"action": "NO_ACTIONS", "details": ""}],
            current_user=access_token["permissions"]["users"][0],
            modals="".join(modals),
            above="",
            below=forms.get_form(
    ...
    @admin_gui.get(
        "/tokens", response_class=HTMLResponse, send_token=True, include_in_schema=False
    )
    async def admin_tokens(access_token=None):
        ...
        for token in tokens:
            ...
            actions = buttons.get_split_button(
                f"view", icon="eye", modal=f"view_{token_number}"
            ) + buttons.get_split_button(
                f"revoke",
        ...
        return server.admin.table_page(
            ...
            if len(tokens) > 0
                ...
                {"username": "NO USER ISSUED TOKENS", "issued": "", "expiration": ""}
            ],
            current_user=access_token["permissions"]["users"][0],
            modals="".join(modals),
            above="",
    ...
    @admin_gui.get(
        "/email", response_class=HTMLResponse, send_token=True, include_in_schema=False
    )
    async def admin_email(access_token=None):
        ...
        SEND_ACTIVATION_EMAILS = (
            email_config[0]["send_activation_emails"] if email_config else ""
        )
        ...
        return server.admin.admin_page(
            ...
            + buttons.get_split_button(
                f"Send Test Email", icon="envelope", modal=f"SendTestEmailModal"
            ),
            current_user=access_token["permissions"]["users"][0],
            modals="".join(modals),
            google=await server.get_google_oauth_client_id(),
    ...
    async def admin_login(request: Request):
        ...
        return await server.get_login_page(message="Login to begin", request=request)
    ...
    @admin_gui.get(
        "/oauth", response_class=HTMLResponse, send_token=True, include_in_schema=False
    )
    async def admin_oauth(access_token=None):
        ...
        for config in oauth_config:
            ...
            default_groups = html_input.get_checkbox(
                "default_groups",
                [
                    (group.group_name, group in config.default_groups)
                    for group in groups
                ],
                size=12,
                unique_id=provider,
